...
-- Script date 1/25/2013 11:03:57 AM
...
AS
	select `so`.`screen_object_id` AS `screen_object_id`,`so`.`screen_id` AS `screen_id`,`so`.`object_id` AS `object_id`,`so`.`control_id` AS `control_id`,`screen`.`object_name` AS `screen_name`,`control`.`object_name` AS `control_name`,`o`.`object_name` AS `object_name`,`controltype`.`object_type` AS `control_type`,`controlbasetype`.`object_type` AS `control_base_type`,`o`.`last_updated` AS `last_updated`,`o`.`last_state_change` AS `last_state_change`,timestampdiff(MINUTE,`o`.`last_state_change`,now()) AS `time_in_state`,`state`.`state_name` AS `state_name` from ((((((`osae_screen_object` `so` join `osae_object` `screen` on((`screen`.`object_id` = `so`.`screen_id`))) join `osae_object` `control` on((`control`.`object_id` = `so`.`control_id`))) join `osae_object_type` `controltype` on((`controltype`.`object_type_id` = `control`.`object_type_id`))) join `osae_object_type` `controlbasetype` on((`controlbasetype`.`object_type_id` = `controltype`.`base_type_id`))) join `osae_object` `o` on((`o`.`object_id` = `so`.`object_id`))) join `osae_object_type_state` `state` on((`o`.`state_id` = `state`.`state_id`)));
...
INSERT INTO osae_object VALUES 
  ...
  (0000000043, 'SYSTEM', 'SYSTEM', 0000000033, 0000000039, NULL, '', 0000000043, 1, '2013-01-25 10:56:57', NULL),
...
INSERT INTO osae_object_property VALUES 
  ...
  (0000000582, 0000000043, 0000000276, '0.4.0', '2013-01-25 10:56:57', NULL),
