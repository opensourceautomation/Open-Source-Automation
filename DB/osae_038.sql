...
-- Script date 5/30/2012 6:21:05 PM
...
--
CREATE DEFINER = 'osae'@'%'
PROCEDURE osae_sp_object_property_set(
  IN  pname        varchar(200),
  IN  pproperty    varchar(200),
...
--
CREATE DEFINER = 'osae'@'%'
PROCEDURE osae_sp_object_state_set(
  IN  pname        varchar(200),
...
--
CREATE DEFINER = 'osae'@'%'
PROCEDURE osae_sp_object_state_set_by_address(
  IN  paddress     varchar(400),
  IN  pstate       varchar(50),
  IN  pfromobject  varchar(200),
  IN  pdebuginfo   varchar(2000)
...
--
CREATE DEFINER = 'osae'@'%'
PROCEDURE osae_sp_object_type_clone(
...
AS
	select `osae`.`osae_object_property_history`.`history_id` AS `history_id`,`osae`.`osae_object_property_history`.`history_timestamp` AS `history_timestamp`,`osae`.`osae_object_property_history`.`property_value` AS `property_value`,`osae`.`osae_object`.`object_name` AS `object_name`,`osae`.`osae_object_property_history`.`object_property_id` AS `object_property_id`,`osae`.`osae_object_type_property`.`property_name` AS `property_name`,`osae`.`osae_object_type_property`.`property_datatype` AS `property_datatype`,`osae`.`osae_object_type`.`object_type` AS `object_type`,`osae`.`osae_object_type`.`object_type_description` AS `object_type_description`,`osae`.`osae_object`.`object_description` AS `object_description` from ((((`osae_object_property_history` join `osae_object_property` on((`osae`.`osae_object_property_history`.`object_property_id` = `osae`.`osae_object_property`.`object_property_id`))) join `osae_object` on((`osae`.`osae_object_property`.`object_id` = `osae`.`osae_object`.`object_id`))) join `osae_object_type_property` on((`osae`.`osae_object_property`.`object_type_property_id` = `osae`.`osae_object_type_property`.`property_id`))) join `osae_object_type` on(((`osae`.`osae_object`.`object_type_id` = `osae`.`osae_object_type`.`object_type_id`) and (`osae`.`osae_object_type_property`.`object_type_id` = `osae`.`osae_object_type`.`object_type_id`))));
...
INSERT INTO osae_object VALUES 
  ...
  (0000000043, 'SYSTEM', 'SYSTEM', 0000000033, 0000000039, NULL, '', 0000000043, 1, '2012-05-30 18:21:05', NULL),
...
INSERT INTO osae_object_property VALUES 
  ...
  (0000000582, 0000000043, 0000000276, '0.3.8', '2012-05-30 16:53:57', NULL),
