...
namespace UserControlDescriptionEditor
{
    public partial class EditorForm : Form
    {
        ...
        public EditorForm(string[] args)
        {
            ...
            if (args != null && args.Length == 1)
            {
                if (File.Exists(Path.GetFullPath(args[0])))
                {
                    ...
                    if (filename.EndsWith("osaud", StringComparison.Ordinal))
                    {
                        // its a UserControl description
        ...
        private void FormText()
        {
            ...
            this.Text = "OSA UserControl Description Editor " + minus + filename + star;
        }
        ...
        private void createPackageToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            // check: safe file needed? (this could also add or remove a osaud file)
            ...
            // check: only one .osaud
            int mopdCount = 0;

            string[] sa0 = Directory.GetFiles(folder, "*.osaud", SearchOption.TopDirectoryOnly);
            ...
            if (mopdCount != 1)
            {
                ...
                if (mopdCount == 0)
                    errortext += "The root folder contains no usercontrol description file.";

                if (mopdCount > 1)
                    errortext += "The root folder contains multiple usercontrol description files of the same type.";
            ...
            string s = Path.GetFileNameWithoutExtension(filename) + ".osauc";
            string[] sa3 = Directory.GetFiles(folder, s, SearchOption.AllDirectories);
            if (sa3.Length > 0)
            {
                DialogResult dr = MessageBox.Show(
                    string.Format(
                            "The folder contains a usercontrol package with the same name as the one to be created." +
                            Environment.NewLine +
            ...
            if (!string.IsNullOrEmpty(folder) && !string.IsNullOrEmpty(filename))
                package = PackagePlugin(Path.Combine(folder, filename), ".osauc");
